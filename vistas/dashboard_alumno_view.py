...
class DashboardAlumnoView(tk.Toplevel):
    ...
    def _create_stat_box(self, parent, title, value):
        """Función auxiliar simplificada para crear un cuadro de estadística."""
    ...
    def _create_error_graph(self, parent, error_data, title):
        """Función auxiliar para crear la gráfica de barras de errores."""
        ...
        if errores_filtrados:
            ...
            ax.text(0.5, 0.5, "Sin errores registrados.", horizontalalignment='center', verticalalignment='center')
    ...
    def create_widgets(self):
        ...
        pruebas_col_frame.grid_rowconfigure(1, weight=1)
        ...
        if self.stats_data.get('total_pruebas', 0) > 0:
            self._create_stat_box(kpi_pruebas_frame, "Intentos Realizados", self.stats_data['total_pruebas']).pack(fill="x", pady=2)
            self._create_stat_box(kpi_pruebas_frame, "Mejor Calificación", f"{self.stats_data['mejor_nota_prueba']:.2f}%").pack(fill="x", pady=2)
            self._create_stat_box(kpi_pruebas_frame, "Nivel Más Común", self.stats_data['nivel_comun_prueba']).pack(fill="x", pady=2)
            ...
            ttk.Label(pruebas_col_frame, text="Aún no has realizado pruebas de práctica.", font=("Arial", 12)).pack(pady=20, padx=10)
        ...
        examenes_col_frame.grid_rowconfigure(1, weight=1)
        ...
        if self.stats_data.get('mejor_nota_examen', 0) > 0 or self.stats_data.get('nivel_comun_examen') != 'N/A':
            self._create_stat_box(kpi_examenes_frame, "Intentos Realizados", f"{self.stats_data.get('total_examenes', 'N/A')}").pack(fill="x", pady=2) # Necesitamos añadir total_examenes a la DB
            self._create_stat_box(kpi_examenes_frame, "Mejor Calificación", f"{self.stats_data['mejor_nota_examen']:.2f}%").pack(fill="x", pady=2)
            self._create_stat_box(kpi_examenes_frame, "Nivel Más Común", self.stats_data['nivel_comun_examen']).pack(fill="x", pady=2)
            ...
            ttk.Label(examenes_col_frame, text="Aún no has realizado exámenes finales.", font=("Arial", 12)).pack(pady=20, padx=10)

        # Botón para cerrar
        ttk.Button(main_frame, text="Cerrar", command=self.destroy).grid(row=1, column=0, columnspan=2, pady=10)